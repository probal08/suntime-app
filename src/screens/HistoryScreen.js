...
import { ArrowLeft, Clock, Sun, User, Calendar, TrendingUp } from 'lucide-react-native';
                        ...
                        <Animated.View style={styles.sessionListContainer} entering={FadeInDown.delay(200).duration(500)}>
                            ...
                            {sessions.length === 0 ? (
                                ...
                                sessions
                                    ...
                                    .map((session, index) => {
                                        // Calculate score for legacy sessions (or use stored)
                                        let score = session.exposureScore;
                                        let statusColor = '#9E9E9E';

                                        if (score === undefined || score === null) {
                                            const result = calculateExposureScore(
                                                session.duration || session.exposureTime || 0,
                                                session.uvIndex || 0,
                                                session.skinType || 3,
                                                session.sunscreen || false,
                                                session.cloudy || false
                                            );
                                            score = result.score;
                                            statusColor = result.color;
                                        } else {
                                            // Determine color based on stored score
                                            if (score < 40) statusColor = '#2196F3';
                                            else if (score <= 80) statusColor = '#4CAF50';
                                            else if (score <= 120) statusColor = '#FF9800';
                                                ...
                                                >
                                                    ...
                                                    <View style={styles.sessionDetails}>
                                                        <View style={styles.detailItem}>
                                                            ...
                                                            <Text style={styles.detailValue}>{session.duration || session.exposureTime}m</Text>
                                                        ...
                                                        {/* Exposure Score */}
                                                        <View style={styles.detailItem}>
                                                            <TrendingUp size={16} color={statusColor} style={{ marginBottom: 4 }} />
                                                            <Text style={[styles.detailValue, { color: statusColor }]}>{score}</Text>
